...
<script lang="ts">
  ...
  let pageTitle = $derived.by(() => {
    if ($page.url.pathname === '/') return null; // Home has its own greeting
    ...
    if ($page.url.pathname === '/track') return null; // Track page has its own header
    // For detail pages (like /paquetes/123 or /facturas/456), don't show title
    if ($page.url.pathname.startsWith('/paquetes/') || $page.url.pathname.startsWith('/facturas/')) {
      return null;
    }
    return null; // Don't show company name as fallback
  });
  let showPageTitle = $derived(pageTitle !== null);
</script>
...
{#if isAuthenticated}
  ...
  <div class="min-h-screen bg-gray-50">
    ...
    <aside class="hidden lg:fixed lg:inset-y-0 lg:flex lg:w-72 lg:flex-col">
      <div class="flex grow flex-col gap-y-5 overflow-y-auto bg-gray-50 px-6 py-8">
        <!-- Logo -->
        <div class="flex h-16 shrink-0 items-center px-2">
          {#if data.logo}
            <img src={data.logo} alt={data.companyName} class="h-10 w-auto" />
          {:else}
            <div class="flex items-center gap-3">
              <div class="w-10 h-10 rounded-xl bg-gradient-to-br from-gray-700 to-gray-800 flex items-center justify-center">
          ...
          <ul role="list" class="flex flex-1 flex-col gap-y-1">
            {#each navItems as item}
              <li>
                <a
                  href={item.path}
                  class="group flex items-center gap-x-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-all
                         {isActive(item.path)
            ...
            <li class="mt-auto pt-4 border-t border-gray-200">
              <form method="POST" action="/logout" use:enhance>
                <button
                  type="submit"
                  class="group flex w-full items-center gap-x-3 rounded-xl px-3 py-2.5 text-sm font-medium text-gray-600 hover:bg-red-50 hover:text-red-600 transition-all"
                >
                  <div class="flex items-center justify-center w-9 h-9 rounded-lg bg-gray-100 group-hover:bg-red-100 transition-colors">
                    <svg class="h-5 w-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="sticky top-0 z-40 lg:hidden">
      <div class="flex h-16 items-center gap-x-4 border-b border-gray-200 bg-white px-4 shadow-sm sm:gap-x-6 sm:px-6">
        <!-- Logo (only show logo, not company name) -->
        {#if data.logo}
          <img src={data.logo} alt={data.companyName} class="h-8 w-auto" />
        {:else}
          <div class="flex items-center gap-2">
            <div class="w-8 h-8 rounded-lg bg-gradient-to-br from-gray-700 to-gray-800 flex items-center justify-center">
    ...
    <!-- Main Content -->
    <main class="lg:pl-72">
      <div class="px-4 py-6 sm:px-6 lg:px-8 lg:py-8 pb-32 lg:pb-8">
        <!-- Page Title (Mobile) -->
        {#if showPageTitle}
